...
const cleanNames = {
  zero_shot: "Zero Shot",
  clinical_substitutions: "Clinical Substitutions",
  DMS_substitutions: "DMS Substitutions",
  DMS_indels: "DMS Indels",
  clinical_indels: "Clinical Indels",
  supervised: "Supervised",
};
const viewTypeMessages = {
  full: "Individual View",
...
const cleanColumns = {
  ...
  MSA_Neff_L_category: "MSA Neff/L Category",
  Model_rank: "Rank",
  Model_name: "Model name",
  Low_MSA_depth: "Low depth",
  Medium_MSA_depth: "Medium depth",
  High_MSA_depth: "High depth",
  "Model details": "Description",
  Function_Activity: "Activity",
  Function_Binding: "Binding",
  Function_Expression: "Expression",
  Function_OrganismalFitness: "Organismal Fitness",
  Function_Stability: "Stability",
  Taxa_Human: "Human",
  Taxa_Other_Eukaryote: "Other Eukaryote",
  Taxa_Prokaryote: "Prokaryote",
  Taxa_Virus: "Virus",
};
...
function Benchmarks() {
  ...
  if (location.state === null) {
    location.state = {
      viewType: "aggregate",
      dataDomain: "DMS_substitutions",
      modelParadigm: "zero_shot",
      sortKey: "Rank-ASC",
      currStatistic: "Spearman",
    };
  }
  const [tableData, setTableData] = useState([]);
  const [viewType, setViewType] = useState(location.state.viewType); // 'full' or 'aggregate'
  const [modelParadigm, setModelParadigm] = useState(
    location.state.modelParadigm,
  ); // 'zeroshot' or 'supervised'
  ...
  const [availableMetrics, setAvailableMetrics] = useState([
    ...
    { value: "MCC", label: "MCC" },
    { value: "Top_recall", label: "Top-10% Recall" },
  ]);
  const [availableParadigms, setAvailableParadigms] = useState([
    { value: "zero_shot", label: "Zero-Shot" },
    { value: "supervised", label: "Supervised" },
  ]);
  const [aggregateSuperheaders, setAggregateSuperheaders] = useState([
    { key: `${currStatistic} by Function`, colspan: 5 },
    { key: `${currStatistic} by MSA Depth`, colspan: 3 },
    { key: `${currStatistic} by Taxon`, colspan: 4 },
    { key: `${currStatistic} by Mutation Depth`, colspan: 5 },
  ...
  function handleCsvData(data, statistic) {
    ...
    if (modelParadigm === "zero_shot") {
      if (dataDomain === "DMS_substitutions") {
        superheaders = [
          { key: `${statistic} by Function`, colspan: 5 },
          { key: `${statistic} by MSA Depth`, colspan: 3 },
          { key: `${statistic} by Taxon`, colspan: 4 },
          { key: `${statistic} by Mutation Depth`, colspan: 5 },
          { key: "Model Details", colspan: 2 },
        ];
        columns =
      ...
      if (dataDomain === "DMS_substitutions" || dataDomain === "DMS_indels") {
        superheaders = [
          { key: `${statistic} by Cross-Validation Scheme`, colspan: 3 },
          { key: `${statistic} by Function`, colspan: 5 },
          { key: `${statistic} by MSA Depth`, colspan: 3 },
          { key: `${statistic} by Taxon`, colspan: 4 },
          { key: "Model Details", colspan: 2 },
        ...
        dataDomain === "clinical_substitutions" ||
        dataDomain === "clinical_indels"
      ) {
        superheaders = [];
        columns =
          `Rank,Model name,Model type,Avg. ${statistic},Std. Error of Diff. to Best Score*`.split(
    ...
    if (viewType === "aggregate") {
      data = data.map((row) => {
        return fixAggregateViewColumns(row, statistic);
      });
      if (data.length > 0) {
  ...
  const sortedTableData = useMemo(() => {
    if (tableData.length === 0) {
      ...
      if (!tableData[0].hasOwnProperty(decodedKey.sortKey)) {
        ...
        return tableData
          .filter((item) => !(item["Rank"] === "" || item["DMS id"] === ""))
          .slice()
  ...
  function customHeader() {
    if (viewType == "full") {
      return (
        <tr style={{ backgroundColor: BACKGROUND_HEADER_COLOR }}>
          {tableColumns.map((column) => (
            <th
              className={
                column === "DMS ID" || column == "RefSeq ID"
                  ? "th-fixed-header"
                  : "default-header"
              }
      ...
      viewType === "aggregate" &&
      (dataDomain === "DMS_substitutions" || dataDomain === "DMS_indels")
    ) {
      return (
        <>
          <tr style={{ backgroundColor: BACKGROUND_HEADER_COLOR }}>
            {tableColumns.map((column, index) => {
              if (index < 5) {
                if (column === "Model name") {
                  return (
                    <th
                      className="th-fixed-header"
                      onClick={() =>
                        setSortKey(computeNextSortKey(sortKey, column))
                      }
                      rowSpan={2}
                      key={column}
                    >
      ...
      return (
        <tr style={{ backgroundColor: BACKGROUND_HEADER_COLOR }}>
          {tableColumns.map((column) => (
            <th
              className={
                column === "Model name" ? "th-fixed-header" : "default-header"
              }
              onClick={() => setSortKey(computeNextSortKey(sortKey, column))}
              key={column}
            >
    ...
    <div className="main-div">
      ...
      <div className="search-and-buttons">
        <div className="dropdowns">
          <Select
            style={{ float: "left", marginRight: "2vw" }}
            label=""
            placeholder="Mutation Type"
            onChange={setDataDomain}
            data={[
              { value: "DMS_substitutions", label: "DMS Substitutions" },
              { value: "DMS_indels", label: "DMS Indels" },
              {
                value: "clinical_substitutions",
                label: "Clinical Substitutions",
              },
              { value: "clinical_indels", label: "Clinical Indels" },
            ]}
            value={dataDomain}
          ></Select>
          <Select
            ...
            label=""
            placeholder="Model Paradigm"
            onChange={setModelParadigm}
            data={availableParadigms}
            value={modelParadigm}
          ></Select>
          <Select
            style={{ float: "right" }}
        ...
        {viewType && (
          <div className="viewtype-buttons">
            <Radio.Group
              name="View Type"
              onChange={(event) => setViewType(event)}
            >
              <Group>
                <Radio
                  value="full"
                  checked={getChecked("full")}
                  label="Individual View"
                ></Radio>
                <Radio
                  value="aggregate"
                  checked={getChecked("aggregate")}
      ...
      <br />
      {filteredSortedTableData.length > 0 ? (
        <TableVirtuoso
          ref={virtuosoRef}
        ...
        <h2 style={{ paddingTop: "6vh" }}>
          {currStatistic} {viewTypeMessages[viewType]} for{" "}
          {cleanNames[modelParadigm]} {cleanNames[dataDomain]} not available
        </h2>
      )}
      {filteredSortedTableData.length > 0 ? (
